...
// Submit request for a raw socket descriptor.
  ...
  char* strHello[20];
  memset(strHello, 0, 20);
// Send ethernet frame to socket.
  //if ((bytes = sendto (sd, ether_frame, frame_length, 0, (struct sockaddr *) &device, sizeof (device))) <= 0) {
  for (i = 0; i < 20; i++) {
      usleep(1000);
  snprintf(strHello, 20, "%d\0", i);
  if ((bytes = sendto (sd, strHello, strlen(strHello), 0, (struct sockaddr *) &device, sizeof (device))) <= 0) {
  //if ((bytes = send(sd, strHello, strlen(strHello), 0)) <= 0) {
  ...
  char buf[bufLen];
  memset(buf, 0, bufLen);
  socklen_t addrlen = sizeof(device);
  printf("wait recv\n");
  for (i = 0; i < 100; i++) {
  if ((bytes = recvfrom(sd, (void*) buf, 100, 0, (struct sockaddr *) &device, &addrlen)) <= 0) {
